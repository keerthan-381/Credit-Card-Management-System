  ...
  const fetchCards = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/get-cards/${user.customer_id}`
      );
      const data = await response.json();
      if (data.cards) {
        setCards(data.cards); // Update cards state with the fetched data
      }
    } catch (error) {
  ...
  return (
    <div className="user-details">
      <nav className="navbar">
        ...
        <Link to="/">
          <button className="back-button">Log out</button>
        </Link>
      </nav>
          ...
          <div className="profile-header">
            ...
            <div className="profile-info">
              ...
              <div className="profile-detail">
                ...
                <p>
                  <strong>ID:</strong> {user.customer_id}
                </p>
              </div>
        ...
        <div className="cards-section">
              ...
              <tbody>
                {cards.map((card, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      ₹{parseFloat(card.outstanding || 0).toFixed(2)}
                    </td>{" "}
                    {/* Ensure numeric value */}
                    <td>
                      <Link to={`/transaction-details/${card.card_number}`}>
          ...
          {isFormVisible && (
            <form onSubmit={handleFormSubmit} className="card-form">
              <input  name="customer_id" value={user.customer_id} />
              <input
                type="text"
                name="card_number"
